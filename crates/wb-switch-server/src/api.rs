...
use wb_switch_core::modules::{
    ...
    credit_usage, credits, export_import, growth, limits, oauth, process, rate_limit_events,
...
pub fn router() -> Router {
    Router::new()
        ...
        // 成长中心（任务/领奖/抽奖/盲盒）
        .route("/api/growth/tasks", get(api_growth_tasks))
        .route("/api/growth/profile", get(api_growth_profile))
        .route("/api/growth/run", post(api_growth_run))
        .route("/api/growth/run-all", post(api_growth_run_all))
        .route("/api/growth/claim-all", post(api_growth_claim_all))
        .route("/api/growth/lottery", post(api_growth_lottery))
        .route(
            "/api/rotate/config",
...
/// 从 query string 取 accountId。
fn query_account_id(query: Option<&str>) -> String {
    query
        .unwrap_or("")
        .split('&')
        .find_map(|pair| {
            let (k, v) = pair.split_once('=').unwrap_or((pair, ""));
            (k == "accountId").then_some(v.to_string())
        })
        .unwrap_or_default()
}

/// GET /api/growth/tasks?accountId= —— 任务列表（结构化）。
async fn api_growth_tasks(RawQuery(query): RawQuery) -> Response {
    let id = query_account_id(query.as_deref());
    if id.is_empty() {
        return json_err("缺少 accountId".to_string(), StatusCode::BAD_REQUEST);
    }
    json_ok(growth::tasks_for_account(&id).await)
}

/// GET /api/growth/profile?accountId= —— 成长档案（等级/能量/连签）。
async fn api_growth_profile(RawQuery(query): RawQuery) -> Response {
    let id = query_account_id(query.as_deref());
    let Some(acc) = account::find_account(&id) else {
        return json_err("账号不存在".to_string(), StatusCode::BAD_REQUEST);
    };
    json_ok(growth::growth_profile(&acc).await)
}

/// POST /api/growth/run {accountId} —— 单账号跑一轮（接受/补领/抽奖/盲盒）。
async fn api_growth_run(Json(body): Json<Value>) -> Response {
    let id = body.get("accountId").and_then(|v| v.as_str()).unwrap_or("");
    json_ok(growth::run_growth_for_account(id).await)
}

/// POST /api/growth/run-all —— 全部国内版账号各跑一轮。
async fn api_growth_run_all() -> Response {
    json_ok(growth::run_growth_all().await)
}

/// POST /api/growth/claim-all {accountId} —— 只补领已完成未领奖任务。
async fn api_growth_claim_all(Json(body): Json<Value>) -> Response {
    let id = body.get("accountId").and_then(|v| v.as_str()).unwrap_or("");
    let Some(acc) = account::find_account(id) else {
        return json_err("账号不存在".to_string(), StatusCode::BAD_REQUEST);
    };
    json_ok(growth::claim_all(&acc).await)
}

/// POST /api/growth/lottery {accountId, maxTimes?} —— 大转盘抽完剩余次数。
async fn api_growth_lottery(Json(body): Json<Value>) -> Response {
    let id = body.get("accountId").and_then(|v| v.as_str()).unwrap_or("");
    let max_times = body.get("maxTimes").and_then(|v| v.as_u64()).unwrap_or(50) as u32;
    let Some(acc) = account::find_account(id) else {
        return json_err("账号不存在".to_string(), StatusCode::BAD_REQUEST);
    };
    json_ok(growth::lottery_draw(&acc, max_times).await)
}
...
async fn static_handler(uri: Uri) -> Response {
    ...
    // 前端路由回退到 index.html
    let dist = concat!(env!("CARGO_MANIFEST_DIR"), "/../../dist");
    let read_file = |p: &str| std::fs::read(format!("{dist}/{p}")).ok();
    let data = read_file(&path).or_else(|| read_file("index.html"));
    match data {
