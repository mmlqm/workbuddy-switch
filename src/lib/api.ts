...
const ROUTES: Record<string, Route> = {
  ...
  growth_profile: { method: "GET", path: "/api/growth/profile" },
  growth_run: { method: "POST", path: "/api/growth/run" },
  growth_run_all: { method: "POST", path: "/api/growth/run-all" },
  growth_claim_all: { method: "POST", path: "/api/growth/claim-all" },
  growth_lottery: { method: "POST", path: "/api/growth/lottery" },
...
// ---------------------------------------------------------------------------
// 成长中心（任务 / 领奖 / 抽奖 / 盲盒）
// 后端挂在 wb-switch-server 的 /api/growth/* 路由；webui 模式直连本地服务。
// ---------------------------------------------------------------------------

/** 成长任务列表（结构化）。 */
export function growthTasks(accountId: string): Promise<{ ok: boolean; tasks: any[]; error?: string }> {
  return call("growth_tasks", { accountId });
}

/** 成长档案（等级/能量/连签）。 */
export function growthProfile(accountId: string): Promise<any> {
  return call("growth_profile", { accountId });
}

/** 单账号跑一轮：接受/补领/抽奖/盲盒。 */
export function growthRun(accountId: string): Promise<any> {
  return call("growth_run", { accountId });
}

/** 全部国内版账号各跑一轮。 */
export function growthRunAll(): Promise<any> {
  return call("growth_run_all");
}

/** 只补领已完成未领奖任务。 */
export function growthClaimAll(accountId: string): Promise<any> {
  return call("growth_claim_all", { accountId });
}

/** 大转盘抽完剩余次数。 */
export function growthLottery(accountId: string, maxTimes = 50): Promise<any> {
  return call("growth_lottery", { accountId, maxTimes });
}
