...
use wb_switch_core::modules::{
    ...
    rotate, session, switch, token_stats, travel, update, variant::WbVariant, growth,
};
...
/// 成长任务列表。
#[tauri::command]
pub async fn growth_tasks(account_id: String) -> Result<Value, String> {
    Ok(growth::tasks_for_account(&account_id).await)
}

/// 成长档案（等级/能量/连签）。
#[tauri::command]
pub async fn growth_profile(account_id: String) -> Result<Value, String> {
    let Some(acc) = account::find_account(&account_id) else {
        return Ok(json!({ "ok": false, "error": "账号不存在" }));
    };
    Ok(growth::growth_profile(&acc).await)
}

/// 单账号跑一轮成长：接受/补进度/领奖/抽奖/盲盒。
#[tauri::command]
pub async fn growth_run(account_id: String) -> Result<Value, String> {
    Ok(growth::run_growth_for_account(&account_id).await)
}

/// 全部国内版账号各跑一轮成长。
#[tauri::command]
pub async fn growth_run_all() -> Result<Value, String> {
    Ok(growth::run_growth_all().await)
}

/// 只补领已完成未领奖任务。
#[tauri::command]
pub async fn growth_claim_all(account_id: String) -> Result<Value, String> {
    let Some(acc) = account::find_account(&account_id) else {
        return Ok(json!({ "ok": false, "error": "账号不存在" }));
    };
    Ok(growth::claim_all(&acc).await)
}

/// 大转盘抽完剩余次数。
#[tauri::command]
pub async fn growth_lottery(account_id: String, max_times: Option<u32>) -> Result<Value, String> {
    let Some(acc) = account::find_account(&account_id) else {
        return Ok(json!({ "ok": false, "error": "账号不存在" }));
    };
    Ok(growth::lottery_draw(&acc, max_times.unwrap_or(50)).await)
}